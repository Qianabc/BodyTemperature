...
double RPPG::outputreference() {
    ...
    ofstream out("sdcard/bpms1.txt");
    for (auto& x : bpms_temp) {
        out << x << endl;
    }
    ...
    for (int j = 0; j < bpms_temp.size() - 4; j++) {
        ...
        for (int i = 0; i < 4; i++) {
            variance += pow(bpms_temp[i+j] - mean, 2);
        }
        variance = variance / 4;
...
double RPPG::temperature(int age,double std_time_diff, double measure_time_diff, int modechoose, int hr_standard,
    ...
    double hr_predict = hr_standard + hr_delta(measure_time_diff, age_flag) - hr_delta(std_time_diff, age_flag);
    int fever_flag;
    ...
    double bt_st = 36.47 + (0.2197 * sin(0.03134 * age + 2.507) + 0.02648 * sin(0.1318 * age - 1.46))
        - 0.1 * cos(measure_time_diff * 0.2661) - 0.39 * sin(measure_time_diff * 0.2661)
        + 0.04 * cos(measure_time_diff * 0.5322) - 0.05 * sin(measure_time_diff * 0.5322) + (0.05 * gender_flag);
